...
let log: XCGLogger = {
    guard let appDelegate = appDelegate else { return XCGLogger.default }
    ...
    let autoRotatingFileDestination = AutoRotatingFileDestination(writeToFile: logPath, identifier: "advancedLogger.fileDestination", shouldAppend: true,
        ...
        maxFileSize: 1024 * 1024 * 5, // 5 megaByte (default is 1 megabyte)
        maxTimeInterval: 60 * 15, // 15 minutes (default is 10 minutes)
        targetMaxLogFiles: 0) // Default is 10, max is 255
    // Optionally set some configuration options
